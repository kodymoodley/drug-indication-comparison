            ...
            while($grow = pg_fetch_array($getSetIDsQueryResult)){                        
                ...
                while (!$globalDone && ($globalIndex < $numAnno)){
                    ...
                    while (!$done && ($index < count($current_annotation_tags))){
                        ...
                        // If the first four or five characters of this tag are "role" or "roles" respectively
                        if ((substr($current_annotation_tags[$index], 0, 4) == "role") || (substr($current_annotation_tags[$index], 0, 5) == "roles")){
                            // This drug does have role tags so add it to the list and stop
                            $done = true;
                            $globalDone = true;
